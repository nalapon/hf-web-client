...
export class IdentityService {
   ...
   */
  private async getUnlockedIdentity(): Promise<{
    key: CryptoKey;
    cert: string;
  } | null> {
    const result = await this.request<{ key: CryptoKey; cert: string } | null>(
      WorkerAction.GetUnlockedIdentity,
      null,
      "password-based",
    );
    if (result.success) {
    ...
    console.warn("getUnlockedIdentity failed: ", result.error.message);
    return null;
  }
  ...
  private async getPrivateKeyPem(key: CryptoKey): Promise<string> {
    ...
    const exportedKeyAsBase64 = btoa(
  ...
  public async exportIdentity(
    ...
    return tryCatch(async () => {
      ...
      if (!this.passwordEngine)
        throw new Error("Password engine not available in this environment.");
      const encryptedResult = await this.passwordEngine.encryptData(
        json,
        password,
      );
   ...
   */
  public async importExportedIdentity(
    encrypted: string,
    password: string,
  ): Promise<Result<void>> {
    return tryCatch(async () => {
      if (!this.passwordEngine)
        throw new Error("Password engine not available in this environment.");
      const decryptedResult = await this.passwordEngine.decryptData(
        encrypted,
        password,
      );
